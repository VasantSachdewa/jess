...
class TestSyncJobsController(unittest.TestCase):
	...
	def test_store_from_queue_to_datastore_valid(
		...
		#mock queue
		queue_obj = MagicMock()
		queue_obj.consume_message = MagicMock(return_value=[MockKafkaMessage(
			'random_id', 'random_message')])
		queue_factory.get_message_queue = MagicMock(
			return_value=queue_obj)
		#mock_extractor
